...
class MagentoContext extends RawMinkContext implements PageObjectAwareInterface
{
     ...
     * @param TableNode $table
     *
     * @Given /^the following Magento configuration:$/
     */
    public function theFollowingMagentoConfiguration(TableNode $rawMageConf)
    {
        $em = $this->getMainContext()->getEntityManager();

        $config = new \Pim\Bundle\MagentoConnectorBundle\Entity\MagentoConfiguration();

        $rawMageConf = $rawMageConf->getHash();
        $indexedConf = [];
        foreach ($rawMageConf as $property) {
            $indexedConf[$property['property']] = $property['value'];
        }

        $config->setCode($indexedConf['code']);
        $config->setLabel($indexedConf['label']);
        $config->setSoapUserName($indexedConf['soapUsername']);
        $config->setSoapApiKey($indexedConf['soapApiKey']);
        $config->setSoapUrl($indexedConf['soapUrl']);
        $config->setDefaultStoreView($indexedConf['defaultStoreView']);
        $config->setDefaultLocale($indexedConf['defaultLocale']);
        $config->setRootCategoryMapping(json_decode($indexedConf['rootCategoryMapping'], true));
        $config->setStoreViewMapping(json_decode($indexedConf['storeViewMapping'], true));
        $config->setAttributeMapping(json_decode($indexedConf['attributeMapping'], true));
