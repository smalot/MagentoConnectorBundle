...
class NormalizerGuesserSpec extends ObjectBehavior
{
    function let(
        ...
        $this->beConstructedWith($channelManager, $mediaManager, $productValueNormalizer, $categoryMappingManager, $associationTypeManager, $productValueManager);
        MagentoSoapClient::getInstance($clientParameters);
    ...
    function it_shoulds_guess_the_product_normalizer_for_parameters($clientParameters, $magentoSoapClient)
    {
        MagentoSoapClient::getInstance()->willReturn($magentoSoapClient);

        $magentoSoapClient->call('core_magento.info')->willReturn(array('magento_version' => '1.8'));

        $this->getProductNormalizer(Argument::any(), true, 4, 'EUR')->shouldBeAnInstanceOf('Pim\Bundle\MagentoConnectorBundle\Normalizer\ProductNormalizer');
    }

    function it_should_return_an_old_version_if_soap_give_an_old_version($clientParameters)
    {
        $magentoSoapClient = MagentoSoapClient::getInstance($clientParameters);
    ...
    function it_raises_an_exception_if_the_version_number_is_not_well_formed($clientParameters, ProductNormalizer $productNormalizer, PriceMappingManager $priceMappingManager)
    {
        $magentoSoapClient = MagentoSoapClient::getInstance($clientParameters);
        MagentoSoapClient::getInstance($clientParameters)->willReturn($magentoSoapClient);
    ...
    function it_shoulds_guess_the_configurable_normalizer_for_parameters($clientParameters, ProductNormalizer $productNormalizer, PriceMappingManager $priceMappingManager)
    {
        $magentoSoapClient = MagentoSoapClient::getInstance($clientParameters);
        MagentoSoapClient::getInstance($clientParameters)->willReturn($magentoSoapClient);
    ...
    function it_raises_an_exception_if_the_version_is_not_supported($clientParameters, ProductNormalizer $productNormalizer, PriceMappingManager $priceMappingManager)
    {
        $magentoSoapClient = MagentoSoapClient::getInstance($clientParameters);
        MagentoSoapClient::getInstance($clientParameters)->willReturn($magentoSoapClient);
    ...
    function it_shoulds_guess_the_category_normalizer_for_parameters($clientParameters, ProductNormalizer $productNormalizer, PriceMappingManager $priceMappingManager)
    {
        $magentoSoapClient = MagentoSoapClient::getInstance($clientParameters);
        MagentoSoapClient::getInstance($clientParameters)->willReturn($magentoSoapClient);
    ...
    function it_shoulds_guess_the_option_normalizer_for_parameters($clientParameters)
    {
        $magentoSoapClient = MagentoSoapClient::getInstance($clientParameters);
        MagentoSoapClient::getInstance($clientParameters)->willReturn($magentoSoapClient);
    ...
    function it_shoulds_guess_the_attribute_normalizer_for_parameters($clientParameters)
    {
        $magentoSoapClient = MagentoSoapClient::getInstance($clientParameters);
        MagentoSoapClient::getInstance($clientParameters)->willReturn($magentoSoapClient);
    ...
    function it_raises_an_exception_if_the_version_not_initialized($clientParameters)
    {
        $magentoSoapClient = MagentoSoapClient::getInstance($clientParameters);
        MagentoSoapClient::getInstance($clientParameters)->willReturn(null);
